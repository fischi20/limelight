 ...
 */
const getDimmedDecoration = (
  reload: boolean = false
): Readonly<vscode.TextEditorDecorationType> => {
...
const applyFocus = async (editor: vscode.TextEditor) => {
  if (focusedEditors.has(editor.document.uri.toString())) {
    ...
    const hidingRanges = [startOfFile, endOfFile].filter(
      (r) => r !== undefined
    );
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const toggleFocusBlockCommand = vscode.commands.registerCommand(
    "limelight.toggleBlockFocus",
    async () => {
      ...
      try {
        ...
        if (!symbols) {
          vscode.window.showInformationMessage(
            "No language symbols found. Is the language supported?"
          );
          return;
        }
  ...
  const setFocusBlockCommand = vscode.commands.registerCommand(
    "limelight.setBlockFocus",
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        setContextFocus(false);
      ...
      try {
        ...
        if (targetSymbol) {
          const existingRange = focusedEditors.get(
  ...
  const toggleFocusSelectionCommand = vscode.commands.registerCommand(
    "limelight.toggleSelectionFocus",
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        return;
  ...
  const setFocusSelectionCommand = vscode.commands.registerCommand(
    "limelight.setSelectionFocus",
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        return;
  ...
  const clearFocusCommand = vscode.commands.registerCommand(
    "limelight.clearFocus",
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        return;
